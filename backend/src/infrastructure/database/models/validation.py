...
class ValidacionTaller(UUIDPrimaryKeyMixin, Base):
    ...
    falla_real: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    tiempo_diagnostico_minutos: Mapped[int | None] = mapped_column(Integer, nullable=True)
    prediccion_correcta: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
