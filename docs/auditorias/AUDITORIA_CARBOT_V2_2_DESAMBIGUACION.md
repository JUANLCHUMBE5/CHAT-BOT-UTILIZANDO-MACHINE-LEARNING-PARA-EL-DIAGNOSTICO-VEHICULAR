# AUDITORÍA TÉCNICA Y FORENSE — CARBOT V2.2
...
FROZEN_PRIMARY_TOP1 (B1)         : 0.8169 (81.69%)
FROZEN_SECONDARY_TOP1 (B2)       : 0.9071 (90.71%)
FROZEN_TERTIARY_TOP1 (B3)        : 0.9071
FROZEN_MEDIA_TOP1                : 0.8770
FROZEN_MEDIA_MACRO_F1            : 0.8678

V2_1_B_PRIMARY_TOP1 (B1)         : 0.8224 (82.24%)
V2_1_B_SECONDARY_TOP1 (B2)       : 0.8907 (89.07%)
V2_1_B_TERTIARY_TOP1 (B3)        : 0.8798
V2_1_B_MEDIA_TOP1                : 0.8643
V2_1_B_MEDIA_MACRO_F1            : 0.8562

BEST_V2_2                        : V2_2_A
BEST_V2_2_PRIMARY_TOP1 (B1)      : 0.8306
BEST_V2_2_SECONDARY_TOP1 (B2)    : 0.9016
BEST_V2_2_TERTIARY_TOP1 (B3)     : 0.9454

BEST_V2_2_PRIMARY_MACRO_F1 (B1)  : 0.8297
BEST_V2_2_SECONDARY_MACRO_F1 (B2): 0.8993
BEST_V2_2_TERTIARY_MACRO_F1 (B3) : 0.9379

BEST_V2_2_MEDIA_TOP1             : 0.8925
BEST_V2_2_MEDIA_MACRO_F1         : 0.8889

PRUEBAS ADVERSARIALES CONTRASTIVAS (n=20 pares = 40 casos):
  - CONTRASTIVE_PAIR_ACCURACY    : 70.00% (vs 45.00% Frozen)
  - EVIDENCE_SENSITIVITY_RATE    : 70.00%

ESTRATEGIAS DE COMBUSTIBLE (V2_2_A):
  - SIN FILTRO                   : 0.55% incompatibilidad | Media F1 = 0.8889
  - HARD FILTER                  : 0.00% incompatibilidad | Media F1 = 0.8889
  - SOFT RERANKING (-75% PENALTY): 0.00% incompatibilidad | Media F1 = 0.8900

ANALISIS ESTADISTICO FORMAL (n=732 casos combinados B1+B2+B3):
  - MCNEMAR TEST P-VALUE         : 0.1690 (Diferencia pareada no significativa al 5%)
  - BOOTSTRAP 95% CI TOP-1 V2_2_A: [0.8538, 0.9003]
  - BOOTSTRAP 95% CI TOP-1 FROZEN: [0.8360, 0.8866]

DATOS SINTETICOS CONTROLADOS:
  - REAL_ORIGINAL                : 6,904
  - REAL_EXTERNAL (Aprobados)    : 215 (195 curados V2.1 + 20 contrastivos técnicos V2.2)
  - SYNTHETIC_ADDED              : 140 casos (catalogados en 14 clases deficitarias)
  - SYNTHETIC_PERCENT_DATASET    : 1.93%

RAG_DECISION                     : KEEP_FROZEN_OEM (FAISS congelado intacto; 4 complementos técnicos aislados)
PRODUCTION_ARTIFACTS_MODIFIED    : 0

FINAL_VERDICT                    : V2_2_CANDIDATO_FUERTE
RECOMMENDATION                   : CONSERVAR_V2_2_PARA_FUTURA_VERSION
====================================================================================================
```
...
|---|---|---|---|---|---|---|---|---|---|---|
| **FROZEN (Producción)** | 0.8169 | 0.8095 | 0.9071 | 0.9032 | 0.9071 | 0.8906 | 0.8770 | 0.8678 | 0.55% | 45.00% |
| **V2_1_B** | 0.8224 | 0.8186 | 0.8907 | 0.8886 | 0.8798 | 0.8615 | 0.8643 | 0.8562 | 0.55% | 40.00% |
| **V2_2_A** | 0.8306 | 0.8297 | 0.9016 | 0.8993 | 0.9454 | 0.9379 | 0.8925 | 0.8889 | 0.55% | 70.00% |
| **V2_2_A (Mejor V2.2)** | **0.8306** | **0.8297** | 0.9016 | 0.8993 | **0.9454** | **0.9379** | **0.8925** | **0.8889** | 0.55% | **70.00%** |
| **V2_2_A_SOFT_RERANKING** | **0.8333** | **0.8329** | 0.9016 | 0.8993 | **0.9454** | **0.9379** | **0.8934** | **0.8900** | **0.00%** | **0.00%** |
...
- **Contrastive Pair Accuracy:** El modelo V2_2_A resolvió correctamente **70.00%** de los pares adversariales (ambas ramas diagnósticas acertadas), superando el **45.00%** de FROZEN.
- **Evidence Sensitivity Rate:** Alcanzó un **70.00%**, demostrando que el modelo utiliza activamente los datos de las pruebas metrológicas y no se apoya en meras palabras superficiales.
...
1. **Test de McNemar (Muestras Pareadas):**
   - Aciertos exclusivos Frozen: 21 casos.
   - Aciertos exclusivos V2_2_A: 32 casos.
   - **$p$-value:** `0.1690`.
...
2. **Intervalos de Confianza al 95% (Bootstrap, 1,000 réplicas):**
   - Top-1 V2_2_A: `[0.8538, 0.9003]`
   - Top-1 FROZEN: `[0.8360, 0.8866]`
...
- **USEFUL_COMPLEMENT (4 documentos):** Procedimientos metrológicos específicos de subsistemas modernos (correa Ford Dragón en aceite, actuador turbo VGT, freno neumático camiones). Quedan catalogados para una futura versión de RAG V3.
- **REDUNDANT (67 documentos):** Procedimientos estándar ya cubiertos por los manuales OEM congelados.
...
- **Total Sintéticos en Dataset Final:** 140 registros (1.93% del dataset).
...
### Veredicto Emitido: `V2_2_CANDIDATO_FUERTE`
- **Fundamentación:** La fase V2.2 demostró que los datos contrastivos dirigidos y la arquitectura de features `Word + Char_wb` mejoran la resolución en el banco primario y en pruebas adversariales específicas (70.00% vs 45.00%). Sin embargo, al contrastar estadísticamente en 732 casos independientes contra `CARBOT_PRECAMPO_FROZEN` ($p = 0.1690$), no se evidencia una ventaja poblacional concluyente en generalización que justifique alterar la versión congelada.

### Regla de Parada y Recomendación de Tesis:
- **`CONSERVAR_V2_2_PARA_FUTURA_VERSION`:** Se aplica la **Regla de Parada (Fase 23)**. NO se debe prolongar la optimización persiguiendo mejoras marginales sobre bancos sintéticos.
