...
- **Alineación con el Proyecto**:
  - Definición formal en la matriz de operativización de variables ([explicacion_teorica_operacionalizacion.md](file:///c:/Users/leonc/OneDrive/Desktop/CHAT_BOT_MACHINLEARNING/docs/notas/explicacion_teorica_operacionalizacion.md)).
...
- **Alineación con el Proyecto**:
  - **Sprint 1**: Recolección de síntomas y estructura CSV (`machine_learning/data/dataset_sintomas.csv`).
  - **Sprint 2**: Algoritmo de clasificación supervisada (`backend/src/infrastructure/modelo_ml.py`).
  - **Sprint 3**: Motor de recuperación RAG (`backend/src/infrastructure/motor_rag.py`).
  - **Sprint 4**: Integración Webhook WhatsApp FastAPI + LLM (`backend/src/interfaces/api/v1/endpoints/webhook.py` y `main.py`).
...
- **Alineación con el Proyecto**:
  ...
  - División de manuales técnicos en bloques temáticos delimitados por secciones en `machine_learning/manuals/manual_procedimientos.txt`.
...
- **Alineación con el Proyecto**:
  - Script oficial de entrenamiento: `machine_learning/training/entrenar_modelo.py`.
  - Script de análisis de resultados e indicadores: `machine_learning/training/analizar_resultados_tesis.py`.
  - Generación de reportes de clasificación de Scikit-Learn e imágenes comparativas en `docs/graficas/`.
...
- **Alineación con el Proyecto**:
  - Coincidencia exactísima con la arquitectura modular `backend/src/`:
    - **Capa Presentación**: `backend/src/interfaces/api/v1/`
    - **Capa Aplicación**: `backend/src/core/gestor_diagnostico.py`
    - **Capa IA (Sub-sistema Tripartito)**: ML + RAG + LLM
    - **Capa Datos**: `machine_learning/data/`, `machine_learning/models/`, `machine_learning/manuals/`
...
- **Alineación con el Proyecto**:
  - Automatización del registro operacional y de auditoría en PostgreSQL; el archivo `machine_learning/data/tracker_diagnosticos.csv` se conserva como instrumento experimental exportable.
