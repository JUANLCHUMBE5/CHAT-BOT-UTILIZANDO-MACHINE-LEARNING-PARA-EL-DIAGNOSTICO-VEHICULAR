...
async def test_webhook_mecanico_autorizado_guarda_en_tablas(setup_test_db, monkeypatch):
    ...
    async with AsyncSession(engine, expire_on_commit=False) as session:
        ...
        assert diag.sintoma_original == "Siento un chillido agudo al frenar el auto"
...
async def test_webhook_rechaza_mecanico_no_autorizado(setup_test_db, monkeypatch):
    ...
    assert resultado["status"] == "no_autorizado"
    assert "no registrado" in resultado["motivo"].lower()

    # Comprobar que se envió el mensaje de rechazo informativo por WhatsApp
    assert len(mensajes_enviados) == 1
    assert "Acceso no autorizado" in mensajes_enviados[0][1]
