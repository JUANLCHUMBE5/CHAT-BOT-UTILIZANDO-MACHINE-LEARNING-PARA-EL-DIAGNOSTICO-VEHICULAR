...
export const TesisTablasFichas: React.FC<TesisTablasFichasProps> = ({
  ...
  if (fase === 'contraste') {
    return (
      <table className="notranslate" translate="no" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '12px' }}>
        <thead>
          <tr
            style={{
                ...
                >
                  ...
                  <td style={{ padding: '10px 14px' }}>
                    <div style={{ fontWeight: 600, color: 'var(--text-main)' }}>
                      <span>{pre.placa} · {pre.marca_modelo}</span>
                    </div>
                    <div style={{ fontSize: '11px', color: '#047857' }}>
                      <span>Falla: {pre.falla_real}</span>
                    </div>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    {fichaId === 'ficha1' && (
                      <div>
                        ...
                        <div style={{ fontSize: '10.5px', color: 'var(--text-muted)' }}>
                          <span>Pred: {pre.chatbot_prediccion || 'Manual'}</span>
                        </div>
                      </div>
                    )}
                    {fichaId === 'ficha2' && (
                      <div>
                        ...
                        <div style={{ fontSize: '10.5px', color: 'var(--text-muted)' }}>
                          <span>Ficha manual tradicional</span>
                        </div>
                      </div>
                    )}
                    {fichaId === 'ficha3' && (
                      <span style={{ fontWeight: 600, color: 'var(--text-main)' }}>
                        <span>{pre.tiempo_diagnostico_minutos} min</span>
                      </span>
                    )}
                  </td>
                  <td style={{ padding: '10px 14px', backgroundColor: 'rgba(59, 130, 246, 0.02)' }}>
                    {fichaId === 'ficha1' && (
                      <div>
                        ...
                        <div style={{ fontSize: '10.5px', color: 'var(--text-muted)' }}>
                          <span>CarBot: {post.chatbot_prediccion}</span>
                        </div>
                      </div>
                    )}
                    {fichaId === 'ficha2' && (
                      <div>
                        ...
                        <div style={{ fontSize: '10.5px', color: 'var(--text-muted)' }}>
                          <span>Estandarizado en sistema</span>
                        </div>
                      </div>
                    )}
                    {fichaId === 'ficha3' && (
                      <span style={{ fontWeight: 700, color: 'var(--primary)' }}>
                        <span>{post.tiempo_diagnostico_minutos} min</span>
                      </span>
                    )}
                  </td>
                  <td style={{ padding: '10px 14px', textAlign: 'center' }}>
                    {fichaId === 'ficha1' && (
                      pre.prediccion_correcta === 0 && post.prediccion_correcta === 1 ? (
                        <span style={{ color: '#059669', fontWeight: 700, backgroundColor: '#ecfdf5', padding: '2px 6px', borderRadius: '4px' }}>
                          + Corrigió falla
                        </span>
                      ) : (
                        <span style={{ color: 'var(--text-muted)' }}>Mantiene resultado</span>
                      )
                    )}
                    {fichaId === 'ficha2' && (
                      pre.campos_completos < post.campos_completos ? (
                        ...
                        <span style={{ color: 'var(--text-muted)' }}>Mantiene 8/8</span>
                      )
                    )}
                    {fichaId === 'ficha3' && (
                      <span style={{ color: '#059669', fontWeight: 700, backgroundColor: '#ecfdf5', padding: '2px 6px', borderRadius: '4px' }}>
                        <span>-{pre.tiempo_diagnostico_minutos - post.tiempo_diagnostico_minutos} min</span>
                      </span>
                    )}
                  </td>
                </tr>
        ...
        <tfoot>
          ...
          >
            <td colSpan={2} style={{ padding: '12px 14px' }}>
              <span>RESUMEN PRE vs POST:</span>
            </td>
            <td style={{ padding: '12px 14px' }}>
              {fichaId === 'ficha1' && (
                <span>Pre PPCF: <strong>{prePPCF.toFixed(1)}%</strong> ({preAciertos}/{preCasos.length})</span>
              )}
              {fichaId === 'ficha2' && (
                <span>Pre PRDC: <strong>{prePRDC.toFixed(1)}%</strong> ({preCompletos}/{preCasos.length})</span>
              )}
              {fichaId === 'ficha3' && (
                <span>Pre TPRD: <strong>{preTPRD.toFixed(1)} min</strong> (total {preTiempoSuma} min)</span>
              )}
            </td>
            <td style={{ padding: '12px 14px', backgroundColor: 'rgba(59, 130, 246, 0.04)' }}>
              {fichaId === 'ficha1' && (
                <span>Post PPCF: <strong style={{ color: 'var(--primary)' }}>{postPPCF.toFixed(1)}%</strong> ({postAciertos}/{postCasos.length})</span>
              )}
              {fichaId === 'ficha2' && (
                <span>Post PRDC: <strong style={{ color: 'var(--primary)' }}>{postPRDC.toFixed(1)}%</strong> ({postCompletos}/{postCasos.length})</span>
              )}
              {fichaId === 'ficha3' && (
                <span>Post TPRD: <strong style={{ color: 'var(--primary)' }}>{postTPRD.toFixed(1)} min</strong> (total {postTiempoSuma} min)</span>
              )}
            </td>
            <td style={{ padding: '12px 14px', textAlign: 'center' }}>
              {fichaId === 'ficha1' && (
                <span style={{ padding: '3px 8px', borderRadius: '4px', backgroundColor: '#dcfce7', color: '#15803d', fontWeight: 800 }}>
                  +{(postPPCF - prePPCF).toFixed(1)}% pp
                </span>
              )}
              {fichaId === 'ficha2' && (
                <span style={{ padding: '3px 8px', borderRadius: '4px', backgroundColor: '#dcfce7', color: '#15803d', fontWeight: 800 }}>
                  +{(postPRDC - prePRDC).toFixed(1)}% pp
                </span>
              )}
              {fichaId === 'ficha3' && (
                <span style={{ padding: '3px 8px', borderRadius: '4px', backgroundColor: '#dcfce7', color: '#15803d', fontWeight: 800 }}>
                  -{(preTPRD - postTPRD).toFixed(1)} min ({(((preTPRD - postTPRD) / (preTPRD || 1)) * 100).toFixed(1)}%)
                </span>
              )}
            </td>
          </tr>
