...
interface MecanicosViewProps {
  mecanicos: Mecanico[];
  onRegistrarMecanico: (data: { nombres: string; telefono: string; password: string; rol: MecanicoRol }) => Promise<void>;
  onToggleActivar: (id: string) => void;
  onToggleBloquear: (id: string) => void;
  onEliminarMecanico: (id: string) => Promise<void>;
  onCambiarRol?: (id: string, nuevoRol: MecanicoRol) => void;
}
...
export const MecanicosView: React.FC<MecanicosViewProps> = ({
  ...
  const [rol, setRol] = useState<MecanicoRol>('mecanico');
  ...
  const handleSubmitRegistro = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(getErrorMessage(error, 'Error al registrar el mecánico en el servidor.'));
    } finally {
      setGuardando(false);
    }
  };
  ...
  const handleConfirmarEliminacion = async () => {
    ...
    try {
      ...
      setMensajeExito(`Se revocó el acceso técnico de ${nombreGuardado}. Ahora vuelve a ser cliente.`);
      setTimeout(() => setMensajeExito(null), 4000);
    } catch (error: unknown) {
      setModalError(getErrorMessage(error, 'No se pudo conectar con el servidor backend FastAPI.'));
    } finally {
      setEliminando(false);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                >
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    {onCambiarRol ? (
                      <select
                        value={m.rol}
                        onChange={(e) => onCambiarRol(m.id, e.target.value as MecanicoRol)}
                        style={{
                          padding: '4px 8px',
                          borderRadius: '6px',
                  ...
                  <td style={{ padding: '14px 16px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '8px' }}>
                      ...
                      {/* Revocar acceso conserva el historial y devuelve al rol cliente */}
                      <Button
                        variant="danger"
                        size="sm"
      ...
      >
        <form onSubmit={handleSubmitRegistro} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <Select
            ...
            value={rol}
            onChange={(e) => setRol(e.target.value as MecanicoRol)}
            options={[
      ...
      {/* Modal de revocación de acceso técnico */}
      {mecanicoAEliminar && (
        <Modal
          ...
          title="Confirmar retiro de acceso técnico"
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <p style={{ fontSize: '14px', color: 'var(--text-main)', lineHeight: 1.5 }}>
              ¿Desea quitar el acceso de mecánico a <strong>{mecanicoAEliminar.nombres}</strong> ({mecanicoAEliminar.telefono})?
            </p>
            <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
              Volverá a ser cliente y conservará sus conversaciones y diagnósticos anteriores.
            </p>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '12px' }}>
              ...
              >
                {eliminando ? 'Actualizando...' : 'Sí, quitar acceso'}
              </Button>
            </div>
          </div>
        </Modal>
