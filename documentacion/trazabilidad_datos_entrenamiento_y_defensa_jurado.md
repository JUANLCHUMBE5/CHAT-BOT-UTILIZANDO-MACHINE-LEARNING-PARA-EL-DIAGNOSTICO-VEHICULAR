...
El proceso reproducible está en `training/preparar_candidatos_zenodo.py` y
`training/traducir_candidatos_es_peru.py`.
...
2. **Prueba sintética de cobertura:** frases generadas o controladas para
   ...
   `data/evaluacion_sintetica_cobertura.csv` pertenece a esta categoría.
...
4. Guardar la evidencia en almacenamiento privado y referenciarla desde la
   plantilla `data/plantilla_evaluacion_externa.csv`.
5. Mantener esos casos fuera del entrenamiento.
6. Ejecutar `training/evaluar_modelo_externo.py` y reportar F1 por clase,
   exactitud, cobertura, solapamientos y bloqueos de producción.

## Evidencias que se pueden mostrar al jurado

- `data/reporte_calidad_dataset.json`.
- `data/candidatos_revision/zenodo_15626055_reporte.json`.
- `data/candidatos_revision/zenodo_15626055_es_peru.csv`.
- `documentacion/notas/fuentes_externas_entrenamiento.md`.
- `models/metricas_modelo.json` para resultados internos.
- `models/metricas_externas.json` para comprobar que la producción permanece
