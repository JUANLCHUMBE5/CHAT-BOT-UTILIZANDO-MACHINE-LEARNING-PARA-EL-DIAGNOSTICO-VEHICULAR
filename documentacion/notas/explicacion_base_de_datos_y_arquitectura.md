...
- `data/dataset_sintomas.csv`: dataset versionable utilizado para entrenar y evaluar el clasificador.
- `manuales_taller/manual_procedimientos.txt`: corpus técnico utilizado por el motor RAG local.
- `models/*.pkl`: artefactos producidos por el entrenamiento; se cargan en memoria para inferencia.
...
1. **Interfaces (`src/interfaces/`)**: recibe webhooks y peticiones HTTP.
2. **Core (`src/core/`)**: coordina reglas y flujo del diagnóstico.
3. **Infraestructura (`src/infrastructure/`)**: adapta ML, RAG, PostgreSQL y servicios externos.
4. **Persistencia (`src/infrastructure/database/`)**: contiene conexión, modelos y sesiones SQLAlchemy; Alembic mantiene el esquema.
