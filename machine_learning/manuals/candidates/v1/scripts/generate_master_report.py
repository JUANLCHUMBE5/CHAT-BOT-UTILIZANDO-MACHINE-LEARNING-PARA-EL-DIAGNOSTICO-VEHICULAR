"""
Script para compilar el Informe Maestro Final: FASE_RAG_IMPLEMENTACION_MAESTRA_REPORTE_FINAL.md
Estructurado estrictamente con las 30 secciones requeridas, la tabla completa de CHG-001 a CHG-020,
la tabla de archivos con SHA-256 pre/post, evidencia para tesis y dictamen técnico final.
"""
import json
import csv
import hashlib
...
def generate_master_report():
    # Hashes congelados de verificación
    FROZEN = {
        "VECTOR_C1": ("machine_learning/models/c1_fase10_final/vectorizador_c1.pkl", "060d0728733499d263f76408b2e99ddb5a56e5dd0f3a006bfa51548397aa96c7"),
        "FAULT_MODEL_C1": ("machine_learning/models/c1_fase10_final/modelo_diagnostico_c1.pkl", "24747fb7d3d465227efbd1376084886b92e5333dd12f0e1b380c0c612585608c"),
        "MACROFIX": ("machine_learning/models/c1_fase10_final/modelo_sistema_c1_macrofix.pkl", "dec3ba707ff000b34c9368935ebe14c30439475910ea82f846cc8e3b9c85930c"),
        "FAISS_BASELINE": ("machine_learning/manuals/indice_faiss.index", "757c4b006a8995f484f539b05420cd929b6034f9aba7e845d305a9d3cf631082"),
        "METADATA_BASELINE": ("machine_learning/manuals/metadatos_manuales.json", "2f0684477522efb67f2b8fbe0e14e5b39cc31bf01423cbafc621a3bc33d76625")
    }

    frozen_checks = []
    for name, (rel_p, expected) in FROZEN.items():
        p = PROJECT_ROOT / rel_p
        actual = hashlib.sha256(p.read_bytes()).hexdigest()
        status = "MATCH (INMUTABLE)" if actual == expected else "FALLO"
        frozen_checks.append((name, rel_p, expected, actual, status))

    # Hashes del Candidato V1
    cand_dir = PROJECT_ROOT / "machine_learning/manuals/candidates/v1"
    CAND_FILES = {
        "FAISS_CANDIDATO": "indexes/indice_faiss_v1.index",
        "METADATA_CANDIDATO_V1": "metadata/metadatos_manuales_v1.json",
        "METADATA_SCHEMA_V2": "metadata/metadatos_schema_v2.json",
        "MANIFEST_CANDIDATO": "manifests/corpus_manifest.json",
        "BUILD_MANIFEST": "manifests/RAG_CANDIDATO_V1_BUILD_MANIFEST.json",
        "DTC_SERVICE": "structured/dtc_lookup_service.py"
    }
    cand_hashes = {}
    for k, rel in CAND_FILES.items():
        p = cand_dir / rel
        if p.exists():
            cand_hashes[k] = hashlib.sha256(p.read_bytes()).hexdigest()
        else:
            cand_hashes[k] = "N/A"

    # Cargar datos de benchmark y ablación
    ablation_rows = []
    with open(cand_dir / "reports/RAG_CANDIDATO_V1_ABLATION.csv", "r", encoding="utf-8") as f:
        ablation_rows = list(csv.DictReader(f))

    # Cargar datos de fuentes
    sources_rows = []
    with open(cand_dir / "reports/RAG_CANDIDATO_V1_SOURCES.csv", "r", encoding="utf-8") as f:
        sources_rows = list(csv.DictReader(f))

    # Cargar datos de contenido bloqueado
    blocked_rows = []
    with open(cand_dir / "reports/RAG_CANDIDATO_V1_BLOCKED_CONTENT.csv", "r", encoding="utf-8") as f:
        blocked_rows = list(csv.DictReader(f))

    # Cargar datos de CHG
    chg_rows = []
    with open(PROJECT_ROOT / "RAG_RECONCILIACION_CHG001_CHG020.csv", "r", encoding="utf-8-sig") as f:
        chg_rows = list(csv.DictReader(f))

    report = f"""# INFORME FINAL MAESTRO: FASE RAG-IMPLEMENTACIÓN MAESTRA CONTROLADA
...
| `candidates/v1/indexes/indice_faiss_v1.index` | NUEVO | CANDIDATO | No existía | `{cand_hashes['FAISS_CANDIDATO']}` | **CREADO** |
| `candidates/v1/metadata/metadatos_schema_v2.json` | NUEVO | CANDIDATO | No existía | `{cand_hashes['METADATA_SCHEMA_V2']}` | **CREADO** |
| `candidates/v1/manifests/corpus_manifest.json` | NUEVO | CANDIDATO | No existía | `{cand_hashes['MANIFEST_CANDIDATO']}` | **CREADO** |
| `candidates/v1/structured/dtc_lookup_service.py` | NUEVO | CANDIDATO | No existía | `{cand_hashes['DTC_SERVICE']}` | **CREADO** |
...
"""
    ...
    p_out = PROJECT_ROOT / "FASE_RAG_IMPLEMENTACION_MAESTRA_REPORTE_FINAL.md"
    with open(p_out, "w", encoding="utf-8") as f:
        f.write(report)
    print(f"INFORME FINAL MAESTRO GENERADO EXITOSAMENTE EN: {p_out}")
