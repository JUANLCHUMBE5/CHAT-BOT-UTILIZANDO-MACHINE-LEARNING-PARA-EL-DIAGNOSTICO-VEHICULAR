"""
Auditoría exhaustiva de Fase 10 Lote 04 para CarBot.
Verifica 200 registros de Clases 16 a 20 contra todas las restricciones:
...
def main():
    ...
    print(f"=== INICIANDO AUDITORIA LOTE 04: {lote04_path} ===")
    
    # 1. UTF-8 check
    try:
        raw_bytes = lote04_path.read_bytes()
    ...
    df_l4 = pd.read_csv(lote04_path)
    total_reg = len(df_l4)
    print(f"Total registros Lote 04: {total_reg}")
    ...
    assert list(df_l4.columns) == cols_esperadas, f"Columnas no coinciden: {df_l4.columns}"
    ...
    conteo_clases = df_l4["clase_objetivo"].value_counts().to_dict()
    for c in clases_l04_esperadas:
    ...
    conteo_niveles = df_l4["nivel_informacion"].value_counts().to_dict()
    ...
    textos_l4 = [str(t) for t in df_l4["texto_usuario"].tolist()]
    textos_l4_norm = [normalizar_texto(t) for t in textos_l4]
    ...
    vectorizer.fit(textos_l4_norm + textos_ref_norm)
    mat_l4 = vectorizer.transform(textos_l4_norm)
    mat_ref = vectorizer.transform(textos_ref_norm)

    sim_intra = cosine_similarity(mat_l4, mat_l4)
    np.fill_diagonal(sim_intra, 0.0)

    sim_inter = cosine_similarity(mat_l4, mat_ref)
    ...
    for i, row in df_l4.iterrows():
        ...
        dups_intra = [j for j, tn in enumerate(textos_l4_norm) if j != i and tn == texto_norm]
        if dups_intra:
            motivos.append(f"Duplicado exacto intra-lote con {df_l4.iloc[dups_intra[0]]['id']}")
            estado = "RECHAZADO"
        ...
        if max_intra > 0.90:
            motivos.append(f"Near-duplicate intra-lote con {df_l4.iloc[max_intra_idx]['id']} (sim={max_intra:.3f})")
            if max_intra > 0.96:
                estado = "RECHAZADO"
