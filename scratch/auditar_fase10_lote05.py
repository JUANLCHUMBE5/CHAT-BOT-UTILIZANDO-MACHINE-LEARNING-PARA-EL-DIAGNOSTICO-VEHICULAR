"""
Auditoría exhaustiva de Fase 10 Lote 05 para CarBot.
Verifica 240 registros de Clases 21 a 26 contra todas las restricciones:
estructura, coherencia técnica, DTC, niveles L1/L2/L3, duplicados intra e inter-lotes,
near-duplicates, contaminación contra Lotes 1-4/TRAIN/DEV/TEST/G1/G2/FIELD60, y contrastivos.
"""
...
def main():
    ...
    print(f"=== INICIANDO AUDITORIA LOTE 05: {lote05_path} ===")

    # 1. UTF-8 check
    try:
        raw_bytes = lote05_path.read_bytes()
    ...
    df_l5 = pd.read_csv(lote05_path)
    total_reg = len(df_l5)
    print(f"Total registros Lote 05: {total_reg}")
    assert total_reg == 240, f"Error: esperados 240, encontrados {total_reg}"
    ...
    assert list(df_l5.columns) == cols_esperadas, f"Columnas no coinciden: {df_l5.columns}"
    ...
    for c in clases_l05_esperadas:
        assert c in clases_validas, f"Clase no canónica: {c}"
    print("[OK] Las 6 clases objetivo pertenecen exactamente a las 61 canónicas.")

    # 4. Distribución de clases y niveles
    conteo_clases = df_l5["clase_objetivo"].value_counts().to_dict()
    for c in clases_l05_esperadas:
        cant = conteo_clases.get(c, 0)
        assert cant == 40, f"Clase '{c}' tiene {cant} != 40 registros."
    print("[OK] Exactamente 40 registros por clase (6 x 40 = 240).")

    conteo_niveles = df_l5["nivel_informacion"].value_counts().to_dict()
    assert conteo_niveles.get("L1") == 60, f"L1 tiene {conteo_niveles.get('L1')} != 60"
    assert conteo_niveles.get("L2") == 90, f"L2 tiene {conteo_niveles.get('L2')} != 90"
    assert conteo_niveles.get("L3") == 90, f"L3 tiene {conteo_niveles.get('L3')} != 90"
    print("[OK] Distribución obligatoria de niveles: L1=60, L2=90, L3=90.")

    # Distribución de niveles por clase
    for c in clases_l05_esperadas:
        sub_c = df_l5[df_l5["clase_objetivo"] == c]
    ...
    print("[OK] Cada una de las 6 clases tiene exactamente 10 L1, 15 L2, 15 L3.")
    ...
    textos_l5 = [str(t) for t in df_l5["texto_usuario"].tolist()]
    textos_l5_norm = [normalizar_texto(t) for t in textos_l5]
    ...
    vectorizer.fit(textos_l5_norm + textos_ref_norm)
    mat_l5 = vectorizer.transform(textos_l5_norm)
    mat_ref = vectorizer.transform(textos_ref_norm)

    sim_intra = cosine_similarity(mat_l5, mat_l5)
    np.fill_diagonal(sim_intra, 0.0)

    sim_inter = cosine_similarity(mat_l5, mat_ref)
    ...
    for i, row in df_l5.iterrows():
        ...
        dups_intra = [j for j, tn in enumerate(textos_l5_norm) if j != i and tn == texto_norm]
        if dups_intra:
            motivos.append(f"Duplicado exacto intra-lote con {df_l5.iloc[dups_intra[0]]['id']}")
            estado = "RECHAZADO"
        ...
        if max_intra > 0.90:
            motivos.append(f"Near-duplicate intra-lote con {df_l5.iloc[max_intra_idx]['id']} (sim={max_intra:.3f})")
            if max_intra > 0.96:
                estado = "RECHAZADO"
