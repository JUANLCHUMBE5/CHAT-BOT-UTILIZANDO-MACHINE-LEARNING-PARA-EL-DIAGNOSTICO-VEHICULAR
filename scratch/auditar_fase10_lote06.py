"""
Auditoría exhaustiva de Fase 10 Lote 06 (FRENOS) para CarBot.
Verifica 280 registros de Clases 27 a 33 contra todas las restricciones:
estructura, coherencia técnica, DTC, niveles L1/L2/L3, duplicados intra e inter-lotes,
near-duplicates, contaminación contra Lotes 1-5/TRAIN/DEV/TEST/G1/G2/FIELD60, y contrastivos.
"""
...
def main():
    ...
    print(f"=== INICIANDO AUDITORIA LOTE 06: {lote06_path} ===")

    # 1. UTF-8 check
    try:
        raw_bytes = lote06_path.read_bytes()
    ...
    df_l6 = pd.read_csv(lote06_path)
    total_reg = len(df_l6)
    print(f"Total registros Lote 06: {total_reg}")
    ...
    assert list(df_l6.columns) == cols_esperadas, f"Columnas no coinciden: {df_l6.columns}"
    ...
    for c in clases_l06_esperadas:
        assert c in clases_validas, f"Clase no canónica: {c}"
        assert FALLA_A_SISTEMA[c] == "FRENOS", f"Macro no es FRENOS: {FALLA_A_SISTEMA[c]}"
    print("[OK] Las 7 clases objetivo pertenecen exactamente a las 61 canónicas (Macro: FRENOS).")

    # 4. Distribución de clases y niveles
    conteo_clases = df_l6["clase_objetivo"].value_counts().to_dict()
    for c in clases_l06_esperadas:
        cant = conteo_clases.get(c, 0)
        assert cant == 40, f"Clase '{c}' tiene {cant} != 40 registros."
    print("[OK] Exactamente 40 registros por clase (7 x 40 = 280).")

    conteo_niveles = df_l6["nivel_informacion"].value_counts().to_dict()
    assert conteo_niveles.get("L1") == 70, f"L1 tiene {conteo_niveles.get('L1')} != 70"
    assert conteo_niveles.get("L2") == 105, f"L2 tiene {conteo_niveles.get('L2')} != 105"
    assert conteo_niveles.get("L3") == 105, f"L3 tiene {conteo_niveles.get('L3')} != 105"
    print("[OK] Distribución obligatoria de niveles: L1=70, L2=105, L3=105.")

    # Distribución de niveles por clase
    for c in clases_l06_esperadas:
        sub_c = df_l6[df_l6["clase_objetivo"] == c]
    ...
    textos_l6 = [str(t) for t in df_l6["texto_usuario"].tolist()]
    textos_l6_norm = [normalizar_texto(t) for t in textos_l6]
    ...
    vectorizer.fit(textos_l6_norm + textos_ref_norm)
    mat_l6 = vectorizer.transform(textos_l6_norm)
    mat_ref = vectorizer.transform(textos_ref_norm)

    sim_intra = cosine_similarity(mat_l6, mat_l6)
    np.fill_diagonal(sim_intra, 0.0)

    sim_inter = cosine_similarity(mat_l6, mat_ref)
    ...
    dtc_por_clase = {c: 0 for c in clases_l06_esperadas}
    ...
    for i, row in df_l6.iterrows():
        ...
        # Coherencia macro
        if macro != "FRENOS":
            motivos.append(f"Macro {macro} != FRENOS")
            estado = "REVISAR"
        ...
        dups_intra = [j for j, tn in enumerate(textos_l6_norm) if j != i and tn == texto_norm]
        if dups_intra:
            motivos.append(f"Duplicado exacto intra-lote con {df_l6.iloc[dups_intra[0]]['id']}")
            estado = "RECHAZADO"
        ...
        if max_intra > 0.90:
            motivos.append(f"Near-duplicate intra-lote con {df_l6.iloc[max_intra_idx]['id']} (sim={max_intra:.3f})")
            if max_intra > 0.96:
                estado = "RECHAZADO"
    ...
    for c, cnt in dtc_por_clase.items():
        print(f"  {c}: {cnt}")
    print(f"DTC por nivel: L1={dtc_por_nivel['L1']}, L2={dtc_por_nivel['L2']}, L3={dtc_por_nivel['L3']}")
    print(f"Casos L2/L3 SIN DTC: {210 - (dtc_por_nivel['L2'] + dtc_por_nivel['L3'])}")

    # Máximas similitudes
    max_sim_global_inter = df_aud["sim_inter_max"].max()
    max_sim_global_intra = df_aud["sim_intra_max"].max()
    print(f"\nMáxima similitud intra-lote: {max_sim_global_intra:.4f}")
