"""
Auditoría exhaustiva de Fase 10 Lote 11 (CARROCERIA_NEUMATICA - Camiones) para CarBot.
Verifica 120 registros de Clases 59 a 61 contra todas las restricciones:
- Estructura canónica de 16 columnas.
- Coherencia técnica, macro_sistema = 'CARROCERIA_NEUMATICA' (Clases 59, 60, 61).
- Distribución exacta: 3 clases x 40 casos = 120 (10 L1, 15 L2, 15 L3 por clase).
...
- Duplicados intra e inter-lotes (L01 a L10).
- Fuga y contaminación (leakage) contra TRAIN, DEV-60, TEST-100, G1, G2, FIELD-60.
- Validación de dominio camión / frenos de aire (no confundir con circuito hidráulico liviano).
"""
...
def auditar_lote11():
    ...
    print(f"=== INICIANDO AUDITORIA LOTE 11: {lote11_path} ===")

    # 1. UTF-8 check
    try:
        raw_bytes = lote11_path.read_bytes()
    ...
    total_reg = len(df_l11)
    print(f"Total registros Lote 11: {total_reg}")
    assert total_reg == 120, f"Error: esperados 120, encontrados {total_reg}"
    ...
    assert list(df_l11.columns) == cols_esperadas, f"Columnas no coinciden: {df_l11.columns}"
    ...
    clases_l11_esperadas = [keys[58], keys[59], keys[60]]

    for c in clases_l11_esperadas:
    ...
    print("[OK] Las 3 clases objetivo pertenecen exactamente a las 61 canónicas con macros correctos.")

    # 4. Distribución de clases y niveles
    conteo_clases = df_l11["clase_objetivo"].value_counts().to_dict()
    for c in clases_l11_esperadas:
        cant = conteo_clases.get(c, 0)
        assert cant == 40, f"Clase {c} tiene {cant} != 40 registros"
    print("[OK] Cada una de las 3 clases tiene exactamente 40 registros.")

    niveles_conteo = df_l11["nivel_informacion"].value_counts().to_dict()
    assert niveles_conteo.get("L1") == 30, f"L1 total {niveles_conteo.get('L1')} != 30"
    assert niveles_conteo.get("L2") == 45, f"L2 total {niveles_conteo.get('L2')} != 45"
    assert niveles_conteo.get("L3") == 45, f"L3 total {niveles_conteo.get('L3')} != 45"
    print(f"[OK] Distribución de niveles: L1={niveles_conteo['L1']}, L2={niveles_conteo['L2']}, L3={niveles_conteo['L3']}.")

    for c in clases_l11_esperadas:
        sub_c = df_l11[df_l11["clase_objetivo"] == c]
    ...
    print("[OK] Cada una de las 3 clases tiene exactamente 10 L1, 15 L2, 15 L3.")
    ...
    df_l10 = pd.read_csv("dataset_fase10_lote_10.csv")
    ...
    for idx, row in df_l10.iterrows():
        corpus_ref[f"LOTE10_{row['id']}"] = ("LOTE_10", str(row["texto_usuario"]))
    ...
    textos_l11 = [str(t) for t in df_l11["texto_usuario"].tolist()]
    textos_l11_norm = [normalizar_texto(t) for t in textos_l11]
    ...
    vectorizer.fit(textos_l11_norm + textos_ref_norm)
    mat_l11 = vectorizer.transform(textos_l11_norm)
    mat_ref = vectorizer.transform(textos_ref_norm)

    sim_intra = cosine_similarity(mat_l11, mat_l11)
    np.fill_diagonal(sim_intra, 0.0)

    sim_inter = cosine_similarity(mat_l11, mat_ref)
    ...
    for i, row in df_l11.iterrows():
        ...
        dups_intra = [j for j, tn in enumerate(textos_l11_norm) if j != i and tn == texto_norm]
        if dups_intra:
            motivos.append(f"Duplicado exacto intra-lote con {df_l11.iloc[dups_intra[0]]['id']}")
            estado = "RECHAZADO"
        ...
        if max_intra > 0.90:
            motivos.append(f"Near-duplicate intra-lote con {df_l11.iloc[max_intra_idx]['id']} (sim={max_intra:.3f})")
            if max_intra > 0.96:
                estado = "RECHAZADO"
    ...
    audit_csv = Path("fase10_auditoria_lote11.csv")
    ...
    print("\n============================================================")
    print("RESUMEN DE AUDITORIA LOTE 11")
    print("============================================================")
    print(f"Total evaluados: {total_reg}")
    print(f"APROBADOS:       {aprobados} / {total_reg}")
    print(f"REVISAR:         {revisar}")
    print(f"RECHAZADOS:      {rechazados}")
    print(f"Similitud intra-lote máxima: {max_intra_global:.4f} (umbral < 0.90)")
    print(f"Similitud inter-referencias máxima: {max_inter_global:.4f} (umbral < 0.88)")
    print(f"Casos con DTC:   {total_con_dtc} (L1: {dtc_por_nivel['L1']}, L2: {dtc_por_nivel['L2']}, L3: {dtc_por_nivel['L3']})")
    print(f"Contrastivos:    {total_contrastivos} / {total_reg}")
    print("============================================================")

    if aprobados == total_reg and revisar == 0 and rechazados == 0:
        print("[OK] LOTE 11 AUDITADO Y 100% APROBADO.")
        return True
    else:
        print("[ALERTA] Lote 11 requiere revisión.")
        return False


if __name__ == "__main__":
    exito = auditar_lote11()
